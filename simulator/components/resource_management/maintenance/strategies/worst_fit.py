...
def worst_fit(env, maintenance_data):
    """
    Worst-Fit like maintenance strategy (presented by Severo et al. 2020)
    ====================================================================
    ...
    When choosing which servers will host the VMs, this strategy uses the Worst-Fit heuristic.
    ...
    while len(Server.nonupdated()) > 0:
        ...
        for server in servers_to_empty:
            ...
            while len(server.virtual_machines) > 0 and servers_checked <= vms_to_migrate * len(candidate_servers):
                ...
                # which is prioritizing servers with more space remaining
        ...
        maintenance_data.append(collect_metrics(env, 'Worst-Fit', servers_to_patch, servers_being_emptied, migrations_data))
